...
public class StrategiesFinder_SearchByLength {
    public static void main(String[] args) {
        try {
            byte newLength = 44;
            int firstGroup = 1;
            int strategiesGroupSize = 1;
            int groupsCap = 1;
            groupsCap = groupsCap + 1 - firstGroup;
            int strategiesCounterForPrinting = strategiesGroupSize * (firstGroup - 1) + 1;

            Files.createDirectories(Paths.get("Strategies_Length_"+newLength));

            FileWriter strategies = new FileWriter("Strategies_Length_"+newLength+"/Strategies_"+firstGroup+".txt");
            ...
            String lastStrategy = "1121322334353644586597674954887847796998566";
                ...
                for (byte j=1; j<=9; j++) {
                    ...
                    if (isPlaceable(gameGrid, i, j)==1) {   //Controlla se si può mettere una X in una certa cella
                        ...
                        if (strategiesCounter%strategiesGroupSize==0 && strategiesCounter!=0) {
                            ...
                            strategies = new FileWriter("Strategies_Length_"+newLength+"/Strategies_"+(strategiesGroup+firstGroup)+".txt");
                    ...
                    else if (strategy.size()>newLength) {
                        i = strategy.get(strategy.size()-2);
                        j = strategy.getLast();
                        gameGrid[i][j] = false;
                        strategy.removeLast();
